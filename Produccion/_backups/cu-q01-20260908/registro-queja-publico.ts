...
export class RegistroQuejaPublico implements OnInit {
  ...
  descargarAcuse(): void {
    ...
    const nombresEvidencias = [
      this.identificacionArchivo ? `IDENTIFICACION_${this.identificacionArchivo.name}` : null,
      ...this.archivos.map((a) => a.name),
    ...
    const tutorHtml = this.tutorConfirmado
      ? `
        ...
        <table>
          ${fila('Nombre', this.escaparHtml([this.tutor.nombre, this.tutor.apellidoPaterno, this.tutor.apellidoMaterno].filter(Boolean).join(' ')))}
