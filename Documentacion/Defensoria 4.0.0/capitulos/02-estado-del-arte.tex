...
\begin{table}[htbp]
\centering
\caption{Resumen de brechas tecnológicas por categoría}
\label{tab:resumen_brechas}
\small
...
\smallskip
\scriptsize
\textbf{Abreviaturas:} Fed: Federales; Loc: Locales; Int: Internacionales; Móv: Móviles; Acad: Académicos; A. IPN: Alerta IPN existente.\\
$\checkmark$ = Presente / $\times$ = Ausente / * Parcial / ** Solo UNAM.\\
...
% ==============================================================================
% SÍNTESIS DE LA BRECHA TECNOLÓGICA Y VALOR AGREGADO
% ==============================================================================

\subsection{Síntesis de la Brecha Tecnológica y Valor Agregado}

\begin{flushleft}
\justifying

Con base en el análisis comparativo de 16 plataformas a través de múltiples dimensiones técnicas y funcionales, se identifica una \textbf{brecha tecnológica estructural} que justifica el desarrollo de este Trabajo Terminal:

\subsubsection*{Dimensiones de la Brecha Identificada}

\textbf{1. Dimensión Funcional (IA/ML):} Los sistemas existentes son reactivos (repositorios de información), no proactivos. Carecen de inteligencia aplicada para la detección temprana de patrones de conflicto o la automatización del triaje administrativo. Ninguna plataforma gubernamental mexicana incorpora NLP. La propuesta implementa Procesamiento de Lenguaje Natural para clasificación automática y detección de anomalías.

\textbf{2. Dimensión Arquitectónica:} Predominan arquitecturas monolíticas y centralizadas (SIDEC, MP Virtual, SAM), con puntos de falla únicos y baja escalabilidad. Las soluciones internacionales han migrado a SaaS sobre nubes públicas (Azure, AWS), contraviniendo principios de soberanía de datos de instituciones gubernamentales mexicanas. La propuesta adopta una arquitectura de microservicios que permite despliegue granular, escalabilidad horizontal y aislamiento de datos sensibles en infraestructura on-premise del IPN.
...
\textbf{4. Dimensión de Geolocalización:} Solo Incorruptible y Alerta IPN incorporan geolocalización entre todas las plataformas analizadas. Ningún sistema institucional de quejas formales (SIDEC, SAM, UNAM) ofrece esta funcionalidad. La propuesta incorpora geolocalización para identificar patrones espaciales de incidencia dentro de las unidades académicas del IPN.

\textbf{5. Dimensión Normativa:} No existe una plataforma alineada específicamente al Marco Jurídico del Instituto Politécnico Nacional (Ley Orgánica, Reglamento Interno, Manual de Procedimientos de la Defensoría). La UNAM cuenta con sistema propio, pero el IPN carece de un equivalente. La propuesta incorpora las figuras jurídicas propias del derecho administrativo politécnico.

\textbf{6. Dimensión de Ciberseguridad:} La soberanía de datos sensibles no está garantizada en soluciones de terceros. Las plataformas mexicanas utilizan cifrado básico en tránsito (TLS) sin implementar cifrado de extremo a extremo. Whistleblower Software establece el estándar con arquitectura Zero-Knowledge. La propuesta adopta cifrado end-to-end para el contenido de denuncias y evidencias.

\subsubsection*{Valor Agregado de la Propuesta}

La plataforma para la Defensoría de los Derechos Politécnicos no busca competir con soluciones comerciales como Whistlelink o EQS Integrity Line, sino \textbf{adaptar las mejores prácticas internacionales} al \textbf{contexto normativo, técnico y presupuestal del IPN}:

\begin{itemize}
    \item \textbf{Autenticación institucional robusta:} Mediante boleta o número de empleado, combinada con anonimato configurable para casos sensibles.
    
    \item \textbf{Inteligencia de datos mediante NLP:} Para triaje automático de denuncias, detección de patrones y clasificación inteligente.
    
    \item \textbf{Arquitectura de microservicios:} Despliegue granular, escalabilidad horizontal, aislamiento de datos y resiliencia ante fallos.
    
    \item \textbf{Geolocalización:} Identificación de puntos calientes dentro de unidades académicas y administrativas del IPN.
    
    \item \textbf{Comunicación bidireccional anónima:} Canal seguro entre denunciante e investigador sin revelar identidad.
    
    \item \textbf{Cifrado de extremo a extremo:} Protección de datos sensibles bajo estándares internacionales.
    
    \item \textbf{Especialización en derecho politécnico:} Alineación con Ley Orgánica del IPN, Reglamento Interno y Manuales de la Defensoría.
\end{itemize}
