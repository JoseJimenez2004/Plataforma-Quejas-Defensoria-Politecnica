...
% \usepackage{colortbl}
% \usepackage{xcolor}
% \usepackage{adjustbox} % Para ajustar tablas demasiado anchas
...
\hline
\textbf{Escalabilidad} & \textbf{Alta}: Soporte nativo para microservicios con Spring Cloud, service discovery (Eureka) & \textbf{Alta}: Cluster module, PM2, arquitectura orientada a eventos \\
\hline
...
\textbf{Ejemplo de API Gateway con Node.js/Express}:

\begin{lstlisting}[language=Java, caption=Estructura conceptual de API Gateway]
// El API Gateway actúa como punto único de entrada
// Redirige peticiones a los microservicios correspondientes
// Implementa rate limiting, autenticación y logging centralizado
